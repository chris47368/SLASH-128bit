...
Public Class SLASH_128_BIT_ENCRYPTON
    ...
    Function verify_key(iv As Byte())
        ...
        '##start salt subtraction
        Dim salt_count As Integer = 0
        For i As Integer = 0 To 3
            iv(12 + i) = iv(12 + i) Xor salthash.hash(i)
            If iv(12 + i) > 0 Then
                '##key verification checkpoint 1
                salt_count = salt_count + 1
                If salt_count > 1 Then
                    Return False
                    Exit Function
                End If
                '##end key verification checkpoint 1
            End If
        Next
        '##end salt substraction

        '## get raw last block size location value
        Dim lbs_loc As Byte = salthash.hash(4) Mod 8

        '##start lastblock size location finder within salt
        Dim salt_loc As Integer 'last block size location within salt 
        If lbs_loc Mod 2 = 0 Then
            salt_loc = lbs_loc / 2 'fetches/calculates correct lbs location from raw lbs location
        Else
            salt_loc = (lbs_loc - 1) / 2 'fetches/calculates correct lbs location from raw lbs location
        End If
        lastblocksize = iv(12 + salt_loc)
        '##end lastblock size location finder within salt

        '##key verification checkpoint 2
        Dim ck_pt2 As Byte = lastblocksize Mod 16
        If lastblocksize > 15 And ck_pt2 > 0 Then
            Return False
            Exit Function
        End If
        '##end key verification checkpoint 2

        '##start fetches/calculates last block location
        If lbs_loc Mod 2 = 0 Then 'lbs was stored and fetched in first nibble of byte if true, second nibble if false
            'true/first nibble
            salt_loc = lbs_loc / 2 'finds correct salt byte
            iv(12 + salt_loc) = lastblocksize
        Else
            'false/last nibble
            salt_loc = (lbs_loc - 1) / 2 'finds correct salt byte
            iv(12 + salt_loc) = iv(12 + salt_loc) Xor lastblocksize 'insert lbs into salt
            iv(12 + salt_loc) = lastblocksize 'reads raw lbs
            lastblocksize = lastblocksize / 16
            End If
        '##end fetches/calculates last block location
